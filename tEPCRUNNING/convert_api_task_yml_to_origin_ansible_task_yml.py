...
def convert_file(file_path,file_path2 ):
    os.system('cp '+ file_path + " " + file_path+".api.bk")
    ...
    for index, input_task in enumerate(list_input_tasks, start=1):
        ...
        output_task['name'] = input_task['block'][0]['name'][2:]
        for orderdict in input_task['block']:
            if orderdict.get('include') is None and orderdict.get('name') is None :
                for key in orderdict.keys():
                    if key != "register":
                        output_task[key] = orderdict[key]
...
def conver_folder_roles(path_role):
    ...
    for role in list_roles:
        convert_file(path_role +'/'+ role + '/tasks/main.yml',path_role +'/'+ role + '/tasks/main_origin.yml')
    print(list_roles)
...
if __name__ == "__main__":
    ...
    # convert_file('chrony_main.yml','chrony_main_repo.yml')
